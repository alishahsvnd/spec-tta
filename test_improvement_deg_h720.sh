#!/bin/bash
# Test Improvements D+E+G on H=720
...
OUTPUT_DIR="results/SPEC_TTA_IMPROVEMENT_DEG"
LOG_FILE="test_improvement_deg_h720.log"

echo "=== Testing Improvements D+E+G on H=720 ===" | tee "$LOG_FILE"
...
echo "----------------------------------------" | tee -a "$LOG_FILE"
echo "Configuration for H=720 with D+E+G:" | tee -a "$LOG_FILE"
...
# With D+E+G
try:
    with open("results/SPEC_TTA_IMPROVEMENT_DEG/iTransformer/ETTh2_720.txt") as f:
        content = f.read()
        match = re.search(r'Final MSE: ([\d.]+)', content)
        updates_match = re.search(r'Total Adaptation Updates: (\d+)', content)
        if match:
            deg_mse = float(match.group(1))
            deg_updates = int(updates_match.group(1)) if updates_match else 0
        else:
            deg_mse = None
            deg_updates = 0
except:
...
if deg_mse:
    change_vs_baseline = ((deg_mse - baseline_mse) / baseline_mse * 100)
    print(f"  D+E+G (output-only + adaptive):    MSE = {deg_mse:.4f} ({change_vs_baseline:+.1f}%), Updates = {deg_updates}")
    
    if deg_mse < baseline_mse:
        improvement_pct = ((baseline_mse - deg_mse) / baseline_mse * 100)
        print(f"\n✓ SUCCESS: {improvement_pct:.1f}% better than baseline!")
    elif deg_mse < eg_mse:
        improvement_pct = ((eg_mse - deg_mse) / eg_mse * 100)
        print(f"\n✓ Improvement over E+G: {improvement_pct:.1f}% better")
        if deg_mse < baseline_mse * 1.1:
            print(f"  Close to baseline (within 10%)")
    else:
        print(f"\n⚠ No improvement over E+G alone")
